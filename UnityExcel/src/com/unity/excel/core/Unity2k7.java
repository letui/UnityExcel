...
public class Unity2k7 implements Unity {
	XSSFWorkbook  wb;
	...
	public OutputStream exportUTable(List<? extends Object> list,
		...
		if(list.size()>0){
			...
			List<FieldWrapper> fws=FieldLoader.loadUColumnField(temp.getClass());
			...
			for(int i=0;i<list.size();i++){
				...
				for(int j=0;j<fws.size();j++){
					...
					cell.setCellType(Cell.CELL_TYPE_STRING);
					cell.setCellValue(fws.get(j).getExportValue(list.get(i)).toString());
